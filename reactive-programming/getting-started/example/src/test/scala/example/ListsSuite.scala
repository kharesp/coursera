...
class ListsSuite extends FunSuite {
  ...
  test("sum should return correct value even if some elements are -ve"){
    assert(sum(List(-1,-2,-3,0)) === -6)
  }
  test("sum of a list of all zeroes should be 0"){
    assert(sum(List(0,0,0)) === 0 )
  }
  test("sum should work even for a list with repeated numbers"){
  ...
  test("max should return the largest int value even if some elements are -ve"){
    assert(max(List(-5,-6,-8,0,1,2)) === 2)
  }
  test("max should return 0 for a list of all zeroes"){
    assert(max(List(0,0,0)) === 0)
  }
  test("max should return maximum element even if list has repeated elements"){
